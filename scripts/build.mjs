...
const publishable = rootFiles.filter(
  (name) =>
    /\.(?:html|css|js|xml|txt)$/i.test(name) ||
    name === "_headers" ||
    name === "_redirects"
);
...
console.log(
  `Built ${publishable.length} root files plus assets and resources into dist/.`
);
